...
<#
    ...
    - 0.6.0.0000   (Gary Blake / 2022-02-22) - Initial early access
    - 1.0.0.1002   (Gary Blake / 2022-06-07) - Initial GA release
...
Try {
    if ($PsBoundParameters.ContainsKey("shutdown")) {
        ...
        if ($counter -eq $retries) {
            Write-PowerManagementLogMessage -Type INFO -Message "The vCLS VMs were not shut down within the expected time. Stopping the script execution." -Colour Red
            Exit
        }
